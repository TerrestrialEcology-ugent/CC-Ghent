...
# *** gather() function from the tidyr package in the tidyverse ***
...
(forest.hist <- ggplot(LPD.forest, aes(x = scalepop)) +
   facet_grid(~biome) +
   geom_histogram(aes(fill = biome), colour = "grey40") +
...
do(ggsave(ggplot(., aes(x = estimate)) +
            ...
            filename = paste0(path1, unique(as.character(.$class),
                                          ".pdf")), device = "pdf"))
#this works but gives an error
```
...
```{r}
#this did not work, not taxa.slopes object
...
# You can increase the limit to get more records - 50 takes a couple of minutes
deer.locations <- occ_search(scientificName = "Cervus elaphus", limit = 50,
